...
def plot_behaviour_maxtrix(base_directory, behaviour_matrix, onsets_dictioanry, block_boundaries, stable_windows, selected_trials):
    ...
    for trial in range(number_of_trials):
        ...
        if trial in selected_trials[2]: axis_1.scatter(2950, scatter_y, s=scatter_size, c='tab:purple')
        if trial in selected_trials[3]: axis_1.scatter(2950, scatter_y, s=scatter_size, c='tab:purple')
        if trial in selected_trials[4]: axis_1.scatter(2950, scatter_y, s=scatter_size, c='tab:cyan')
    ...
    for window in stable_windows:
        ...
        window_patch = patches.Rectangle(xy=(2800, window_patch_y), height=window_height, width=200, alpha=0.5, color='gold')
