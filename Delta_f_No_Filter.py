...
def process_pixels(base_directory, delta_f_file):
    ...
    # Load Data
    blue_file_name = get_blue_file(base_directory)
    violet_file_name = get_violet_file(base_directory)

    blue_file_path = os.path.join(base_directory, blue_file_name)
    violet_file_path = os.path.join(base_directory, violet_file_name)

    blue_data_container = h5py.File(blue_file_path, 'r')
    violet_data_container = h5py.File(violet_file_path, 'r')

    blue_matrix = blue_data_container["Data"]
    violet_matrix = violet_data_container["Data"]
    ...
    violet_baseline_frames = np.load(os.path.join(base_directory, "Baseline_Frames", "Violet_Baseline_Frames.npy"))
    blue_baseline_frames = np.load(os.path.join(base_directory,  "Baseline_Frames", "Blue_Baseline_Frames.npy"))
    ...
    with h5py.File(delta_f_file, "w") as f:
        ...
        for chunk_index in range(number_of_chunks):
            ...
            violet_data = violet_matrix[chunk_pixels]
