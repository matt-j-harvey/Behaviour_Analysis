...
def propagate_visual_block(trial_index, behaviour_matrix, accuracy_threshold, irrelevance_threshold):
    ...
    while still_propagating:
        ...
        if trial_index == number_of_trials:
            ...
            if current_trial_type == 2 or current_trial_type == 3:
                still_propagating = False
...
def propagate_odour_block(trial_index, behaviour_matrix, accuracy_threshold):
...
def get_stable_windows(behaviour_matrix):
    ...
    for trial in range(number_of_trials):
        ...
        if trial_type == 1 or trial_type == 2:
            potential_window_size = propagate_visual_block(trial, behaviour_matrix, accuracy_threshold, irrel_threshold)
            potential_window_size_list.append(potential_window_size)

        # Check If Odour
        elif trial_type == 3 or trial_type == 4:
            potential_window_size = propagate_odour_block(trial, behaviour_matrix, accuracy_threshold)
            potential_window_size_list.append(potential_window_size)
