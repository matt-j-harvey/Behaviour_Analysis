...
def plot_visual_performance(meta_group_list, group_names=None, mouse_names=None):
    ...
    for group_index in range(number_of_groups):
        ...
        for mouse_index in range(group_size):
            ...
            for session in mouse:
                ...
                visual_d_prime = performance_dictionary["visual_post_transition_visual_d_prime"]

                visual_performance_list.append(visual_d_prime)
                mouse_id_list.append(mouse_name)
                group_id_list.append(group_name)

    # Combine_Into Dataframe
    dataframe = pd.DataFrame()
    dataframe["Visual Performance"] = visual_performance_list
    ...
    axis = seaborn.swarmplot(y="Visual Performance", hue="Genotype", x="Genotype", data=dataframe, size=8)
    axis.set_ylim(0, 4)
    plt.show()
...
plot_visual_performance([control_session_list, mutant_session_list], group_names=group_names, mouse_names=mouse_names)
